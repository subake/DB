...
class ImageDataset(data.Dataset, Configurable):
    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                ...
                gt_path=[self.data_dir[i]+'/train/gts/'+timg.strip()+'.txt' for timg in image_list]
            else:
                image_path=[self.data_dir[i]+'/test/images/'+timg.strip() for timg in image_list]
                print(self.data_dir[i])
                if 'TD500' in self.data_list[i] or 'total_text' in self.data_list[i]:
                    gt_path=[self.data_dir[i]+'/test/gts/'+timg.strip()+'.txt' for timg in image_list]
                else:
                    gt_path=[self.data_dir[i]+'/test/gts/'+timg.strip().split('.')[0]+'.txt' for timg in image_list]
    ...
    def load_ann(self):
        ...
        for gt in self.gt_paths:
            ...
            for line in reader:
                ...
                elif 'sroie' in self.data_dir[0].lower():
                    # print("sroie!!!")
                    # print(line)
                    poly = np.array(list(map(float, line[:8]))).reshape((-1, 2)).tolist()
                    label = ','.join(parts[8:])
                    # print(label)
                elif 'cord' in self.data_dir[0].lower():
                    # print("cord!!!")
                    # print(line)
                    poly = np.array(list(map(float, line[:8]))).reshape((-1, 2)).tolist()
                    label = ','.join(parts[8:])
    ...
    def __getitem__(self, index, retry=0):
        ...
        data = {}
        image_path = self.image_paths[index] + '.jpg'
        # print(image_path)
